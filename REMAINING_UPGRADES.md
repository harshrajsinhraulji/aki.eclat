...
- **Volumetric Lighting:** A volumetric WebGL light source that casts real-time soft shadows on DOM elements.
- **Glass Refraction:** Glassmorphism that actually distorts the elements behind it based on the index of refraction.
- **Cloth Physics:** Verlet integration cloth physics; moving the mouse creates wind that ripples fabric elements.
- **Particle System Morphing:** Transitions between pages where the current page shatters into 100,000 particles that reassemble into the next page.
- **Depth of Field (DoF):** True camera focal length with WebGL post-processing.
- **Spatial Audio:** Subtle ambient sounds panning from left to right audio channels as elements move across the screen.
- **3D Object Scanning:** Photogrammetry-scanned 3D objects you can physically rotate 360 degrees.
- **Liquid Transitions:** Clicking a link causes the screen to melt.
- **Raymarching (SDF):** Mathematically generated SDF blobs that merge and separate organically.
